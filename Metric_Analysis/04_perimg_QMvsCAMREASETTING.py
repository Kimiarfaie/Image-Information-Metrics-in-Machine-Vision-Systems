...
'''
Script 04:
This script correlates image quality metrics and object detection performance (mAP@[0.50:0.95])
together with different camera settings across multiple networks.

Change necessary variable:

1. networks → list of object detection models 
2. keyword →  camera setting being analyzed
   (options: "ISO", "EV", "size", "sharpness", "NOISE-ONLY")
3. xlabel → label for the x-axis, e.g. "SNR (dB)" or "MTF50 (Cyc/Pxl)".
4. split_conditions → choose which grouping of conditions to use:
       ISO_split, Focused_split, EV_split, size_split, ISOfocuse_split
5. `imatest_summary_path` and `csv_path`

'''
# --- CONFIGURATION ---
networks = ["YOLO11n", "YOLO11m", "SSD", "FasterRCNN-mobilenet", "FasterRCNN"]
keyword = "ISO"  # Change
xlabel = "SNR (dB)"  # Change
iqm_source = "Dataset"
